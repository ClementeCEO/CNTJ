...
export function crearVideoJs(canalId, urlCarga, viewMode = 'grid-view') {
    ...
    if (tipoReproductor === 'shaka' && typeof shaka !== 'undefined') {
        ...
        setTimeout(async () => {
            try {
                ...
                console.error(`[CNTJ] Error at attempt to initialize Shaka Player for channel with id: ${canalId}. Error:`, error);
    ...
    if (tipoReproductor === 'oplayer' && typeof OPlayer !== 'undefined') {
        ...
        setTimeout(() => {
            try {
                ...
                console.error(`[CNTJ] Error at attempt to initialize OPlayer for channel with id: ${canalId}. Error: ${error}`);
                showToast({
    ...
    try {
        ...
        console.error(`[CNTJ] Error at attempt to initialize Video.js for channel with id: ${canalId}. Error: ${error}`);
        showToast({
...
export function crearOverlay(canalId, tipoSeñalCargada, valorIndex = 0) {
    try {
        ...
        console.error(`[CNTJ] Error at attempt to create overlay for channel with id: ${canalId}. Error: ${error}`);
        showToast({
...
export function crearFragmentCanal(canalId, viewMode = 'grid-view') {
    if (channelsList[canalId]?.señales) {
        ...
        console.error(`[CNTJ] Error at attempt to create fragment for channel with id: ${canalId}. Error: ${error}`);
        showToast({
